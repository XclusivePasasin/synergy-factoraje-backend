...
def bitacora(modulo, accion):
    ...
    def decorador(f):
        ...
        def wrapper(*args, **kwargs):
            id_usuario = request.headers.get("id-usuario-bitacora")
            usuario = request.headers.get("nombre-usuario-bitacora")

            # Validamos que la informacion de autenticación esté presente
            if not id_usuario or not usuario:
                return response_error("Faltan datos de autenticación en la solicitud, necesarios para registrar la bitácora.", http_status=401)
